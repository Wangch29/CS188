...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        "*** YOUR CODE HERE ***"
        self.state_action_qvalue: util.Counter = util.Counter()
    ...
    def getQValue(self, state, action):
        ...
        if self.state_action_qvalue[state] == 0:
            return 0.0

        return self.state_action_qvalue[state][action]
    ...
    def update(self, state, action, nextState, reward):
        ...
        "*** YOUR CODE HERE ***"
        if self.state_action_qvalue[state] == 0:
            self.state_action_qvalue[state] = util.Counter()

        max_next_qvalue = self.computeValueFromQValues(nextState)

        self.state_action_qvalue[state][action] = (
                (1 - self.alpha) * self.getQValue(state, action)
                + self.alpha * (reward + self.discount * max_next_qvalue)
        )
